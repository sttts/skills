...
def validate_finding_ids(value: Any, field: str, finding_ids: set[str], *, allow_empty: bool = False) -> list[str]:
    ...
    for index, item in enumerate(related):
        ...
        if finding_id not in finding_ids:
            raise ReviewError(f"{field}[{index}] references unknown finding {finding_id!r}")
...
def validate_score(score: Any, field: str) -> None:
    ...
    total = base
    for index, factor in enumerate(require_list(score.get("factors"), f"{field}.factors")):
        ...
        if not isinstance(delta, int) or isinstance(delta, bool) or not -10 <= delta <= 10:
...
def validate_spec(spec: dict[str, Any], diff: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, finding in enumerate(findings):
        ...
        if len(details) < 2:
            raise ReviewError(f"{prefix}.details must contain at least two labeled groups")
        ...
        steps = require_list(suggested_fix.get("steps"), f"{prefix}.suggested_fix.steps")
        if not steps:
        ...
        for test_index, test in enumerate(require_list(suggested_fix.get("tests", []), f"{prefix}.suggested_fix.tests")):
    ...
    for index, flow in enumerate(flows):
        ...
        validate_finding_ids(flow.get("finding_ids"), f"{prefix}.finding_ids", finding_ids)
    ...
    primary_diagrams = [diagram for diagram in diagrams if diagram.get("primary") is True]
    ...
    for index, diagram in enumerate(diagrams):
        ...
        if primary and related_findings:
            raise ReviewError(f"{prefix}.finding_ids must be empty for the PR-wide primary diagram")
        diagram_type = require_string(diagram.get("type"), f"{prefix}.type")
        if diagram_type not in VALID_DIAGRAM_TYPES:
            raise ReviewError(f"{prefix}.type must be one of {sorted(VALID_DIAGRAM_TYPES)}")
        require_string(diagram.get("title"), f"{prefix}.title")
        require_string(diagram.get("description", ""), f"{prefix}.description", allow_empty=True)
        ...
        if diagram_type == "component":
            ...
            for node_index, node in enumerate(nodes):
                ...
                if node_id in node_ids:
                    raise ReviewError(f"{prefix}.nodes contains duplicate id {node_id!r}")
                node_ids.add(node_id)
                require_string(node.get("label"), f"{node_prefix}.label")
                require_string(node.get("detail", ""), f"{node_prefix}.detail", allow_empty=True)
                kind = node.get("kind", "existing")
                if kind not in VALID_COMPONENT_KINDS:
                    raise ReviewError(f"{node_prefix}.kind must be one of {sorted(VALID_COMPONENT_KINDS)}")
                for axis in ("x", "y"):
                    value = node.get(axis)
                    if value is not None and (
                        ...
                        raise ReviewError(f"{node_prefix}.{axis} must be a number from 0 through 100 when set")
                target = node.get("target")
                if target is not None and target not in ids:
                    raise ReviewError(f"{node_prefix}.target references unknown id {target!r}")
            for edge_index, edge in enumerate(require_list(diagram.get("edges"), f"{prefix}.edges")):
                edge_prefix = f"{prefix}.edges[{edge_index}]"
                if not isinstance(edge, dict):
                    raise ReviewError(f"{edge_prefix} must be an object")
                for endpoint in ("from", "to"):
                    value = require_string(edge.get(endpoint), f"{edge_prefix}.{endpoint}")
                    if value not in node_ids:
                        raise ReviewError(f"{edge_prefix}.{endpoint} references unknown node {value!r}")
                require_string(edge.get("label"), f"{edge_prefix}.label")
                tone = edge.get("tone", "neutral")
                if tone not in VALID_DIAGRAM_TONES:
                    raise ReviewError(f"{edge_prefix}.tone must be one of {sorted(VALID_DIAGRAM_TONES)}")
                for axis in ("label_x", "label_y"):
                    value = edge.get(axis)
                    if value is not None and (
                        not isinstance(value, (int, float)) or isinstance(value, bool) or not 0 <= value <= 100
                    ):
                        raise ReviewError(f"{edge_prefix}.{axis} must be a number from 0 through 100")
                target = edge.get("target")
                if target is not None and target not in ids:
                    raise ReviewError(f"{edge_prefix}.target references unknown id {target!r}")
        else:
            participants = require_list(diagram.get("participants"), f"{prefix}.participants")
            if len(participants) < 2:
                raise ReviewError(f"{prefix}.participants must contain at least two entries")
            participant_names: set[str] = set()
            for participant_index, participant in enumerate(participants):
                value = require_string(participant, f"{prefix}.participants[{participant_index}]")
                if value in participant_names:
                    raise ReviewError(f"{prefix}.participants contains duplicate name {value!r}")
            ...
            for event_index, event in enumerate(events):
                ...
                if source not in participant_names or destination not in participant_names:
                    raise ReviewError(f"{event_prefix} references an unknown participant")
                if source == destination:
                    raise ReviewError(f"{event_prefix} must connect two different participants")
                require_string(event.get("label"), f"{event_prefix}.label")
                require_string(event.get("detail", ""), f"{event_prefix}.detail", allow_empty=True)
                tone = event.get("tone", "neutral")
                if tone not in VALID_DIAGRAM_TONES:
                    raise ReviewError(f"{event_prefix}.tone must be one of {sorted(VALID_DIAGRAM_TONES)}")
                target = event.get("target")
                if target is not None and target not in ids:
                    raise ReviewError(f"{event_prefix}.target references unknown id {target!r}")

    manifest_coverage: set[str] = set()
    for index, manifest in enumerate(manifests):
        prefix = f"manifests[{index}]"
        require_string(manifest.get("title"), f"{prefix}.title")
        require_string(manifest.get("description", ""), f"{prefix}.description", allow_empty=True)
        require_string(manifest.get("language", "yaml"), f"{prefix}.language")
        require_string(manifest.get("code"), f"{prefix}.code")
        related = validate_finding_ids(manifest.get("finding_ids"), f"{prefix}.finding_ids", finding_ids)
        manifest_coverage.update(related)

    for index, finding in enumerate(findings):
        if finding.get("involves_api_objects", False) and finding["id"] not in manifest_coverage:
            raise ReviewError(f"findings[{index}] involves API objects but has no linked manifest example")
    ...
    for index, row in enumerate(matrix):
        ...
        if not isinstance(row, dict):
            raise ReviewError(f"{prefix} must be an object")
        validate_finding_ids(row.get("finding_ids"), f"{prefix}.finding_ids", finding_ids)
    ...
    for index, reference in enumerate(references):
        ...
        validate_finding_ids(reference.get("finding_ids"), f"{prefix}.finding_ids", finding_ids)
    ...
    for index, block in enumerate(code_blocks):
        ...
        validate_finding_ids(block.get("finding_ids"), f"{prefix}.finding_ids", finding_ids)
        ...
        if smell not in VALID_SMELL_LEVELS:
            raise ReviewError(f"{prefix}.smell must be one of {sorted(VALID_SMELL_LEVELS)}")
        smell_reason = require_string(block.get("smell_reason", ""), f"{prefix}.smell_reason", allow_empty=True)
        ...
        if smell != "none" and not smell_reason:
            raise ReviewError(f"{prefix}.smell_reason is required when smell is {smell}")
    ...
    if normalized.get("agent_prompt"):
        raise ReviewError("top-level agent_prompt is not supported; put agent_prompt on each finding")
...
def main() -> int:
    ...
    try:
        ...
        replacements = {
            ...
            "__CYTOSCAPE_JS__": (
                "/*\n"
                + (asset_root / "vendor" / "LICENSE-cytoscape.txt").read_text(encoding="utf-8")
                + "\n*/\n"
                + (asset_root / "vendor" / "cytoscape-3.33.4.min.js").read_text(encoding="utf-8")
            ).replace("</", "<\\/"),
            "__DAGRE_JS__": (
                "/*\n"
                + (asset_root / "vendor" / "LICENSE-dagre.txt").read_text(encoding="utf-8")
                + "\n*/\n"
                + (asset_root / "vendor" / "dagre-0.8.5.min.js").read_text(encoding="utf-8")
            ).replace("</", "<\\/"),
            "__CYTOSCAPE_DAGRE_JS__": (
                "/*\n"
                + (asset_root / "vendor" / "LICENSE-cytoscape-dagre.txt").read_text(encoding="utf-8")
                + "\n*/\n"
                + (asset_root / "vendor" / "cytoscape-dagre-2.5.0.js").read_text(encoding="utf-8")
            ).replace("</", "<\\/"),
        }
        rendered = html
